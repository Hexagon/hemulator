...
- **Features**:
  ...
  - **Hardware-accurate sprite priority**:
    - DMG: X coordinate first (lower X = higher priority), then OAM order
    - CGB: OAM order only (lower index = higher priority)
...
- **136 total tests**:
  - PPU tests (rendering, registers, scrolling, sprite priority)
