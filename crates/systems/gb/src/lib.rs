...
mod tests {
    ...
    #[test]
    fn test_gb_cgb_palette_initialization() {
        // Test that CGB mode initializes palettes with proper grayscale values
        let mut sys = GbSystem::new();
        ...
        // Read palette data to verify initialization
        // The palettes should be initialized with grayscale values, not all white

        // Test background palette 0, color 0 (should be white: 0x7FFF)
        sys.cpu.memory.ppu.write_bgpi(0x80); // Set index to 0 with auto-increment
        let c0_low = sys.cpu.memory.ppu.read_bgpd();
        sys.cpu.memory.ppu.write_bgpi(0x81);
        let c0_high = sys.cpu.memory.ppu.read_bgpd();
        assert_eq!(c0_low, 0xFF, "BG Palette 0 Color 0 low byte should be 0xFF");
        assert_eq!(
            c0_high, 0x7F,
            "BG Palette 0 Color 0 high byte should be 0x7F"
        );

        // Test background palette 0, color 1 (should be light gray: 0x56B5)
        sys.cpu.memory.ppu.write_bgpi(0x82);
        let c1_low = sys.cpu.memory.ppu.read_bgpd();
        sys.cpu.memory.ppu.write_bgpi(0x83);
        let c1_high = sys.cpu.memory.ppu.read_bgpd();
        assert_eq!(c1_low, 0xB5, "BG Palette 0 Color 1 low byte should be 0xB5");
        assert_eq!(
            c1_high, 0x56,
            "BG Palette 0 Color 1 high byte should be 0x56"
        );

        // Test background palette 0, color 2 (should be dark gray: 0x294A)
        sys.cpu.memory.ppu.write_bgpi(0x84);
        let c2_low = sys.cpu.memory.ppu.read_bgpd();
        sys.cpu.memory.ppu.write_bgpi(0x85);
        let c2_high = sys.cpu.memory.ppu.read_bgpd();
        assert_eq!(c2_low, 0x4A, "BG Palette 0 Color 2 low byte should be 0x4A");
        assert_eq!(
            c2_high, 0x29,
            "BG Palette 0 Color 2 high byte should be 0x29"
        );

        // Test background palette 0, color 3 (should be black: 0x0000)
        sys.cpu.memory.ppu.write_bgpi(0x86);
        let c3_low = sys.cpu.memory.ppu.read_bgpd();
        sys.cpu.memory.ppu.write_bgpi(0x87);
        let c3_high = sys.cpu.memory.ppu.read_bgpd();
        assert_eq!(c3_low, 0x00, "BG Palette 0 Color 3 low byte should be 0x00");
        assert_eq!(
            c3_high, 0x00,
            "BG Palette 0 Color 3 high byte should be 0x00"
        );

        // Also verify object palette has the same initialization
        sys.cpu.memory.ppu.write_obpi(0x80);
        let obj_c0_low = sys.cpu.memory.ppu.read_obpd();
        assert_eq!(
            obj_c0_low, 0xFF,
            "OBJ Palette 0 Color 0 low byte should be 0xFF"
        );
    }
