...
impl Tia {
    ...
    pub fn get_ntsc_palette() -> &'static [u32; 128] {
...
mod tests {
    ...
    fn test_timing_constants_accuracy() {
        ...
        assert_eq!(
            HBLANK_CLOCKS + VISIBLE_CLOCKS, COLOR_CLOCKS_PER_SCANLINE,
        ...
        assert_eq!(tia.pixel, 0, "After 76 CPU cycles (228 color clocks), pixel should wrap to 0");
        assert_eq!(tia.scanline, 1, "After one complete scanline, should advance to scanline 1");
    }
    ...
    fn test_wsync_timing_accuracy() {
        ...
        assert_eq!(
            tia.cpu_cycles_until_scanline_end(), 1,
    ...
    fn test_scanline_advancement_timing() {
        ...
        assert_eq!(tia.scanline, 1, "After 76 CPU cycles, should advance to scanline 1");
        assert_eq!(tia.pixel, 0, "After scanline boundary, pixel should reset to 0");
        ...
        assert_eq!(tia.scanline, 0, "After scanline 261, should wrap to scanline 0");
    ...
    fn test_color_clock_to_cpu_cycle_conversion() {
        ...
        assert_eq!(tia.pixel, 78, "26 CPU cycles should advance to pixel 78 (26 × 3)");
    ...
    fn test_hblank_offset_calculation() {
        ...
        tia.pixel = 0;
        assert_eq!(tia.current_visible_x(), 0, "Pixel 0 should map to visible x=0 (clamped)");
        
        // At pixel 67 (end of HBLANK), visible x should still be 0 (clamped)
        tia.pixel = 67;
        assert_eq!(tia.current_visible_x(), 0, "Pixel 67 (HBLANK) should map to visible x=0");
        
        // At pixel 68 (start of visible area), visible x should be 0
        tia.pixel = 68;
        assert_eq!(tia.current_visible_x(), 0, "Pixel 68 should map to visible x=0");
        
        // At pixel 100, visible x should be 32 (100 - 68)
        tia.pixel = 100;
        assert_eq!(tia.current_visible_x(), 32, "Pixel 100 should map to visible x=32");
        
        // At pixel 227 (last visible pixel), visible x should be 159 (227 - 68)
        tia.pixel = 227;
        assert_eq!(tia.current_visible_x(), 159, "Pixel 227 should map to visible x=159");
        
        // Verify HBLANK constant matches implementation
        assert_eq!(Tia::HBLANK_COLOR_CLOCKS, 68, "HBLANK should be 68 color clocks");
    }
}
