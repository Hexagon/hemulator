...
impl RdpRenderer for OpenGLRdpRenderer {
    fn init(&mut self, width: u32, height: u32) {
        unsafe {
            ...
            // Resize textures
            self.gl.bind_texture(glow::TEXTURE_2D, Some(self.color_texture));
            ...
            self.gl.bind_renderbuffer(glow::RENDERBUFFER, Some(self.depth_renderbuffer));
    ...
    fn fill_rect(
        ...
        unsafe {
            ...
            if let Some(loc) = u_color {
                self.gl.uniform_4_f32(Some(&loc), rgba[0], rgba[1], rgba[2], rgba[3]);
            }
