...
mod boot_output_tests {
    ...
    fn test_comprehensive_boot() {
        ...
        let boot_sector = std::fs::read(boot_bin_path).expect("Failed to read comprehensive_boot.bin");
        assert_eq!(
            boot_sector.len(),
            512,
            "Boot sector should be exactly 512 bytes"
        ...
        assert!(
            screen_output.contains("PC Boot Test") || screen_output.contains("CPU") || screen_output.contains("BOOT>"),
