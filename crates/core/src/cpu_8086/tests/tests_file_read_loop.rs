...
fn test_xchg_ax_preserves_upper_bits() {
    ...
    assert_eq!(
        cpu.ax & 0xFFFF_0000,
        0x1234_0000,
        "EAX upper bits should be preserved"
    );
    assert_eq!(cpu.bx & 0xFFFF, 0x0042, "BX should have AX's lower 16 bits");
    assert_eq!(
        cpu.bx & 0xFFFF_0000,
        0x5678_0000,
        "EBX upper bits should be preserved"
    );
}
...
fn test_cwd_preserves_upper_bits() {
    ...
    assert_eq!(
        cpu.dx & 0xFFFF,
        0xFFFF,
        "DX should be 0xFFFF (sign extension)"
    );
    assert_eq!(
        cpu.dx & 0xFFFF_0000,
        0x5678_0000,
        "EDX upper bits should be preserved"
    );
}
