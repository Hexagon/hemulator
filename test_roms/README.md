# Test ROMs

This directory contains minimal test ROMs for smoke testing each emulated system.

**For detailed test ROM requirements and guidelines**, see [AGENTS.md](../AGENTS.md#test-rom-requirements).

## Purpose

These test ROMs are designed to:
1. Verify basic system functionality (CPU, memory, video output)
2. Provide deterministic output for automated testing
3. Serve as reference implementations for new systems

## Current Systems

- **NES** - Full implementation with smoke test
- **SNES** - Minimal implementation with smoke test
- **Game Boy** - Implementation with smoke test and visible output validation
- **Game Boy Color** - DMG-compatible mode with smoke test
- **Atari 2600** - Full implementation with smoke test
- **SMS** - Sega Master System with checkerboard pattern smoke test
- **SG-1000** - Sega SG-1000 with TMS9918A Graphics I mode smoke test
- **N64** - Basic RDP display list implementation with smoke test
- **ColecoVision** - Graphics II mode with multi-color bands and sprites

## Future Systems

When implementing new systems, follow this pattern:
1. Create a subdirectory: `test_roms/<system>/`
2. Write minimal assembly code that produces visible output
3. Add build script and built ROM
4. Add smoke test to system crate
5. Update this README and [AGENTS.md](../AGENTS.md#test-rom-requirements)

## Building

Each system has a `build.sh` script that assembles the test ROM from source:

```bash
# NES
cd nes && ./build.sh

# SNES
cd snes && ./build.sh

# Game Boy
cd gb && ./build.sh

# Atari 2600
cd atari2600 && ./build.sh

# SMS (Sega Master System)
cd sms && ./build.sh

# SG-1000
cd sg1000 && ./build.sh

# ColecoVision
cd colecovision && ./build.sh

# N64
cd n64 && ./build_systemtest.sh
```

## Requirements

- **NES**: cc65 (6502 assembler/linker)
- **SNES**: cc65 (65816 assembler/linker, compatible with 6502 tools)
- **Game Boy**: rgbds (GB assembler/linker)
- **Atari 2600**: dasm (6502 assembler)
- **SMS**: Python 3 (for ROM generation script)
- **SG-1000**: Python 3 (for ROM generation script)
- **ColecoVision**: Python 3 (for ROM generation script)
- **N64**: Rust toolchain + nust64 (`cargo +stable install nust64`)

On Ubuntu/Debian:
```bash
sudo apt-get install cc65 dasm python3

# For rgbds, build from source:
git clone https://github.com/gbdev/rgbds.git
cd rgbds && make && sudo make install
```

## Test ROM Specifications

### NES (test.nes)
- Format: iNES (16-byte header)
- Mapper: 0 (NROM)
- PRG-ROM: 2 x 16KB
- CHR-ROM: 1 x 8KB
- Behavior: Displays checkerboard pattern with alternating white/red tiles
- Expected output: Visible checkerboard pattern on screen

### SNES (test.sfc)
- Format: LoROM (32KB)
- Internal header: At $FFB0-$FFDF
- Vectors: At $FFE0-$FFFF
- Behavior: Initializes PPU, writes tilemap and CHR data, displays pattern
- Expected output: Basic rendering test (infrastructure verification)

### Game Boy (test.gb)
- Format: GB ROM (with Nintendo logo)
- Size: 32KB
- Cartridge type: ROM only (no MBC)
- Behavior: Fills screen with tile $00 (checkerboard pattern)
- Expected output: Visible checkerboard pattern on screen

### Game Boy Color (test.gbc)
- Format: GBC ROM (CGB flag = 0x80, DMG compatible)
- Size: 32KB
- Cartridge type: ROM only (no MBC)
- Behavior: Same as GB test ROM, runs in DMG mode
- Expected output: Visible checkerboard pattern on screen

### Atari 2600 (test.bin)
- Format: Raw binary
- Size: 4KB
- Behavior: Sets playfield to alternating pattern ($AA)
- Expected output: Visible playfield pattern on screen

### Atari 2600 (checkerboard.bin)
- Format: Raw binary
- Size: 4KB
- Behavior: Creates checkerboard pattern by alternating playfield every 2 scanlines
- Expected output: Checkerboard pattern with ~50% white pixels

### Atari 2600 (test_timer.bin)
- Format: Raw binary
- Size: 4KB
- Behavior: Tests RIOT timer interrupt flag clear-on-read behavior
- Expected output: Playfield with changing colors based on timer

### Atari 2600 (game_test.bin)
- Format: Raw binary
- Size: 4KB
- Behavior: Game-like test ROM with realistic patterns:
  - Color bars (8 bars of 8 scanlines each, cycling through hues)
  - Sprite positioning and horizontal movement (two player sprites)
  - Multiple playfield patterns (alternating 0xAA/0x55 and 0x55/0xFF/0x00)
  - Per-scanline background color changes
- Expected output: Multi-section display with color bars, moving sprites, and playfield patterns
- Purpose: Stress-test emulator with realistic game-like behavior to expose timing and rendering issues

### N64 (n64-systemtest)
- **Source**: https://github.com/lemmy-64/n64-systemtest
- **Format**: N64 ROM (built using nust64 tool)
- **Purpose**: Comprehensive hardware test suite for N64 emulators
- **Features**:
  - Self-validating: ROM reports test results directly ("Done! Tests: XXX. Failed: 0")
  - Tests CPU instructions, exceptions, TLB, memory access, RSP
  - Works in known-good emulators (Project64, Mupen64Plus, etc.)
  - Fast execution for regression testing
- **Building**:
  - Install Rust: https://www.rust-lang.org/tools/install
  - Install nust64: `cargo +stable install nust64`
  - Run: `cd test_roms/n64 && ./build_systemtest.sh`
- **Expected output**: Text message on screen with test results
- **Note**: Simple test ROMs removed as they don't work in known-good emulators. Use n64-systemtest for all N64 testing.

### SMS (test.sms)
- Format: SMS ROM with TMR SEGA header
- Size: 32KB
- Header: TMR SEGA signature at 0x7FF0
- Behavior: 
  - Initializes VDP registers (mode control, name table, sprite table)
  - Loads tile 0 (all white pixels) and tile 1 (all black pixels)
  - Fills name table at 0x3800 with alternating tiles (checkerboard pattern)
  - Sets palette: white (0x3F) and black (0x00)
- Expected output: Checkerboard pattern on 256x192 screen (~50% white, 50% black pixels)
- Note: ROM generated by Python script (no external Z80 assembler required)

### SG-1000 (test.sg)
- Format: Raw binary (no header)
- Size: 32KB
- Load Address: $0000 (cartridge ROM space)
- Behavior:
  - Initializes TMS9918A VDP in Graphics I mode
  - Sets up pattern table at $0800 with two patterns (0xAA and 0x55 checkerboard)
  - Sets up color table at $2000 with white on black
  - Fills name table at $1800 with alternating patterns
  - Enables display; sets VDP frame interrupt enable bit (CPU interrupts remain disabled)
- Expected output: Checkerboard pattern on 256x192 screen (Graphics I mode)
- Note: ROM generated by Python script (no external Z80 assembler required)

### ColecoVision (test.col)
- Format: Raw binary cartridge ROM
- Size: 32KB
- Load Address: $8000 (ColecoVision cartridge space)
- BIOS: Requires 8KB BIOS ROM (auto-detected or provided via --bios)
- Behavior:
  - Initializes TMS9918A VDP in Graphics II mode
  - Loads tile patterns (256 solid tiles)
  - Sets up color table with 4 horizontal bands (white, red, green, cyan)
  - Fills nametable with sequential tiles
  - Creates 2 sprites (yellow and magenta)
  - Enables display with frame interrupts
- Expected output: 4 colored horizontal bands + 2 sprites on 256x192 screen
- Note: ROM generated by Python script (no external Z80 assembler required)

## Integration with Tests

These ROMs are included in the smoke tests for each system crate:
- `crates/systems/nes/src/lib.rs` - NES smoke test
- `crates/systems/snes/src/lib.rs` - SNES smoke test
- `crates/systems/gb/src/lib.rs` - Game Boy and Game Boy Color smoke tests
- `crates/systems/atari2600/src/lib.rs` - Atari 2600 smoke test
- `crates/systems/sms/src/system.rs` - SMS smoke test
- `crates/systems/sg1000/src/lib.rs` - SG-1000 smoke test
- `crates/systems/colecovision/src/lib.rs` - ColecoVision smoke test (manual VDP initialization)

**Note**: N64 smoke tests removed. The simple test ROMs don't work in known-good emulators. For N64 testing, use n64-systemtest manually: `cargo run --profile release-quick -- test_roms/n64/n64-systemtest.n64`

The tests load each ROM (or manually construct equivalent state for N64 and ColecoVision), run it for a few frames (or process display lists), and verify that the output frame contains expected non-zero pixel data (for systems with working rendering) or correct dimensions (for minimal implementations), confirming that the emulator infrastructure is functioning correctly.
