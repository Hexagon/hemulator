...
- [ ] **Per-draw texture re-upload (major perf bottleneck)**: Every textured triangle call (`draw_triangle_textured_zbuf` / `draw_triangle_textured_shaded_zbuf`) re-uploads the entire CPU-sampled texture to the GPU via `tex_image_2d`. Needs TMEM dirty tracking: detect when `LOAD_TILE`/`LOAD_BLOCK` updates TMEM, decode the tile once into a GPU texture, and reuse it for all triangles with the same tile descriptor. Also consider batching all triangles into a single draw call per texture. — `crates/systems/n64/src/rdp_renderer_opengl.rs` (`upload_dynamic_texture`), `crates/systems/n64/src/rdp.rs`
