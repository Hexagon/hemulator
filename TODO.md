...
### High
- [x] **RDP Blend/Combine pipeline**: ~~`SET_OTHER_MODES` values are stored but never consumed by the rendering pipeline.~~ Implemented: (1) Z-buffer depth now correctly applied to `gl_Position` so GPU depth ordering matches the N64 scene depth; (2) new `draw_triangle_textured_shaded_zbuf` method applies colour-combiner MODULATE mode (TEXEL0 × SHADE) — RSP HLE passes per-vertex shade colours for lit textures; (3) alpha blending (src-alpha / 1-src-alpha) is automatically enabled when the FORCE_BL bit is set in `othermode_l`. Remaining: cycle type, full multi-cycle combine. — `crates/systems/n64/src/rdp.rs`, `crates/systems/n64/src/rdp_renderer_opengl.rs`, `crates/systems/n64/src/rsp_hle.rs`
...
- [x] **Per-draw texture re-upload (major perf bottleneck)**: ~~Every textured triangle call re-uploads the entire CPU-sampled texture to the GPU via `tex_image_2d`.~~ Implemented TMEM dirty tracking: `notify_tmem_loaded()` is called from `load_texture_to_tmem()` and `LOAD_TLUT`. The OpenGL renderer now skips both CPU sampling and `tex_image_2d` when TMEM is unchanged and tile dimensions match, reusing the existing GPU texture. Batching all triangles into a single draw call remains as a future improvement. — `crates/systems/n64/src/rdp_renderer_opengl.rs` (`upload_dynamic_texture`, `notify_tmem_loaded`), `crates/systems/n64/src/rdp.rs`
- [ ] **Synchronous glReadPixels stalls GPU pipeline**: `sync_framebuffer()` uses a blocking `glReadPixels` call which forces the GPU pipeline to drain completely before proceeding. Replace with double-buffered Pixel Buffer Objects (PBOs) so the readback is async: initiate readback at end of frame N, read the result at the start of frame N+1. — `crates/systems/n64/src/rdp_renderer_opengl.rs` (`read_pixels`)
- [ ] **No vertex batching — one draw call per primitive**: Each `draw_triangle_*` call submits a separate `glDrawArrays`. Collect all draw commands for a frame into a vertex buffer and issue a single batched draw call per shader/state group. — `crates/systems/n64/src/rdp_renderer_opengl.rs`
...
- [x] **RDP performance counters**: ~~`DPC_CLOCK`, `DPC_BUFBUSY`, `DPC_PIPEBUSY`, and `DPC_TMEM` registers return hardcoded 0.~~ All four counters now increment on each processed RDP command: `dpc_clock` +10/cmd, `dpc_pipebusy` +1 for triangle/rect cmds, `dpc_bufbusy` by total-commands-executed, `dpc_tmem` +1 per LOAD_TILE/LOAD_BLOCK. — `crates/systems/n64/src/rdp.rs`
...
- [ ] **RDP cycle type / full colour combiner**: Cycle-1 MODULATE (A=TEXEL0, C=SHADE), PRIM_MODULATE (C=PRIMITIVE), and ENV_MODULATE (C=ENVIRONMENT) patterns are now decoded correctly with fixed bit extraction (mul_rgb0 at bits 47:43). 1-cycle, 2-cycle, copy, and fill cycle types are partially implemented. Full COMBINED/SHADE/TEXEL1/NOISE/CENTER/K5 mux support still needed. — `crates/systems/n64/src/rdp.rs`
- [x] **RDP texture filtering**: ~~SET_OTHER_MODES texture filter bits (point/bilinear/median) are stored but not applied.~~ `set_othermode()` now parses `sample_type` (bits 11-10 of othermode_h): value 2 (2×2 average) enables `GL_LINEAR`; others use `GL_NEAREST`. `execute_rdp_command` for `SET_OTHER_MODES` (0x2F) now delegates to `set_othermode()` so direct RDP display-list paths also apply the setting. — `crates/systems/n64/src/rdp.rs`, `crates/systems/n64/src/rdp_renderer_opengl.rs`
- [x] **SI (Serial Interface) DMA**: ~~SI DMA transfers were instant.~~ Now deferred by `SI_DMA_DELAY_CYCLES` (~750 cycles, ~8 µs at 93.75 MHz). SI_STATUS bit 0 reports DMA-busy during the delay; the MI_INTR_SI interrupt fires exactly when the counter reaches zero. SI_STATUS bit 12 mirrors the interrupt pending state. — `crates/systems/n64/src/bus.rs`
- [ ] **RDRAM configuration registers (0x03F0_0000–0x03FF_FFFF)**: Reads/writes to RDRAM internal registers (device type, mode, latency, etc.) return placeholder values; some fields (device_id, mode) need accurate values from hardware specifications. Some homebrew and bootcode probes these registers. — `crates/systems/n64/src/bus.rs`
...
- [x] **RI (RDRAM Interface) registers**: ~~Register reads at 0x0470_0000–0x047F_FFFF return 0.~~ Now return realistic placeholder values (RI_MODE=0x0E, RI_CONFIG=0x40, RI_REFRESH=0x63634). Writes silently accepted. — `crates/systems/n64/src/bus.rs`
- [x] **RDRAM boundary check in word/doubleword access**: ~~`read_word` / `write_word` for RDRAM can panic at end of RDRAM.~~ `read_word` already uses `.get(offset..offset+4)` with fallback; `read_doubleword` now has a RDRAM fast path using safe slice reads. — `crates/systems/n64/src/bus.rs`
- [x] **OpenGL state toggling overhead**: ~~`enable/disable(SCISSOR_TEST)` and `enable/disable(DEPTH_TEST)` are called per-draw-call.~~ Added per-field state cache (`scissor_test_enabled`, `depth_test_enabled`, `blend_enabled`, `current_scissor`) with `set_scissor_test()`, `set_depth_test()`, `set_blend()`, `apply_scissor()` helpers that only emit GL calls on actual state transitions. — `crates/systems/n64/src/rdp_renderer_opengl.rs`
- [x] **Bus read_doubleword decomposed**: ~~`read_doubleword` calls `read_word` twice, each translating the address.~~ Added a RDRAM fast path (phys_addr ≤ 0x3FFFF8) that reads both words directly from the slice. — `crates/systems/n64/src/bus.rs`
