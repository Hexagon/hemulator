...
- **`cpu_8086`**: Intel 8086 CPU implementation with comprehensive instruction set
  ...
  - **8086/8088 Base Implementation**:
    ...
    - **ModR/M addressing support**: Full decoder with all addressing modes
      - Register mode (mod=11)
      - Memory modes with 8/16-bit displacements (mod=00, 01, 10)
      - All EA calculations: [BX+SI], [BX+DI], [BP+SI], [BP+DI], [SI], [DI], [BP], [BX], direct address
    - **Data movement**:
    ...
    - **Arithmetic operations**:
      ...
      - SUB immediate (0x2C-0x2D)
      - SUB with ModR/M (0x28-0x2B) - register to/from register/memory
    ...
    - **Control flow**:
      ...
      - RET near (0xC3) and with immediate (0xC2)
      - RET far (0xCB) and with immediate (0xCA)
...
- ✅ **Game Boy** - Fully Functional (all core features including audio)
...
- **Game Boy (`emu_gb`)**: Functional implementation with PPU, joypad, and rendering
  ...
  - **Features**:
    ...
    - Audio synthesis integrated with frontend (44.1 kHz output)
  ...
  - All tests pass (80 tests: 13 PPU, 7 APU, 8 system, 41 mapper, 11 timer tests)
