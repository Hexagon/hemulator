...
2. **Run a debug dump** — once the logic looks right, run the full binary to capture a trace or dump.
   The first build is slow (~2.5min), but subsequent runs reuse cached artifacts and generally rebuild
   only the affected crates (typically the system crate + `emu_gui`):
   ```bash
